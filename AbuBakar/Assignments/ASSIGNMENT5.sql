...
CREATE VIEW v_pending_processing_orders AS
SELECT
o.order_id,
...
SELECT * FROM v_pending_processing_orders;
...
(net_sales - previous_month_sales) AS revenue_difference
FROM (
SELECT
MONTH(o.order_date) AS order_month,
SUM(oi.quantity * oi.list_price * (1 - oi.discount)) AS net_sales,
LAG(SUM(oi.quantity * oi.list_price * (1 - oi.discount)), 1) OVER (ORDER BY MONTH(o.order_date)) AS previous_month_sales
FROM sales.orders o
JOIN sales.order_items oi ON o.order_id = oi.order_id
